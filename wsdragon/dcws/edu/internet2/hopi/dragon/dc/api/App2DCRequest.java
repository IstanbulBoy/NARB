...
/**
 * Contains the parameters of a DC API application request. 
 * DC application requests are used to retrieve a path between 
 * two endpoints and optionally available VLAN tags. 
 * App2DCRequests are issued to the DC server by DC clients.
 * 
 * @author Andrew Lake (alake@internet2.edu)
 */
public class App2DCRequest {
	 ...
	 */
	public App2DCRequest(String src, String dst, byte encoding, byte swcap, byte[] gpidNum, float bwidth) throws UnknownHostException{
		int bodyLength = 0;
		/* Set Type */
		type = new byte[2];
		type[0] = 0x00;
		...
		/* Create header */
		header = new DCMessageHeader(DCMessageHeader.TYPE_LSPQ, bodyLength + 4, null, DCMessageHeader.VTAG_ANY);
		
		/* Sum total size of packet */
		packetSize = bodyLength + DCMessageHeader.SIZE + 4;
	}
	 ...
	 * @throws UnknownHostException returned when an invalid source or destination is given
	 */
	public App2DCRequest(String src, String dst, float bwidth) throws UnknownHostException{
		this(src, dst, ENCODING_ETHERNET, SWCAP_L2SC, GPID_ETHERNET, bwidth);
	}
	
	/**
	 * Generates a DC API message packet (including header) in raw byte form from this object.
	 * @return byte array containing a DC API message packet (including header)
	 */
	public byte[] toBytes(){
	 ...
	 * @return the DC API header of this message
	 */
