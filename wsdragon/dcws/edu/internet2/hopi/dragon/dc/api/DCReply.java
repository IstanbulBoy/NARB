...
/**
 * Represents a reply from the DC server. It can be generated 
 * by passing specific values or from the raw bytes of a packet 
 * returned by a DC server. It also contains many usefule static 
 * constants related to fields found in DC reply messages.
 * 
 * @author Andrew Lake (alake@internet2.edu)
 *
 */
public class DCReply {
	private DCMessageHeader header;
	private int type;
	private int length;
	...
	/* TLV types */
	public final static int TLV_TYPE_DC_REQUEST = 0x02;
	public final static int TLV_TYPE_DC_ERO = 0x03;
	public final static int TLV_TYPE_DC_ERROR_CODE = 0x04;
	public final static int TLV_TYPE_DC_REQUEST_VTAG_MASK = 0x05;
	...
	/**
	 * Constructor that creates a DCReply object from passwed parameters for individual fields
	 * 
	 * @param header a DCMessageHEader that represents the header of this new reply
	 ...
	 */
	public DCReply(DCMessageHeader header, int type, int length, ArrayList<EROSubobject> ero, ArrayList<Integer> vtagList, int errorCode) {
		this.header = header;
		this.type = type;
		this.length = length;
		this.ero = ero;
	...
	/**
	 * Constructor that creates a DCReply object from the raw bytes of a packet returned by a DC server
	 ...
	 */
	public DCReply(byte[] packet) throws UnknownHostException{
		header = new DCMessageHeader(packet);
		
		/* Set TLV type */
		type = packet[DCMessageHeader.SIZE] << 8;
		type += packet[DCMessageHeader.SIZE + 1];
		
		/* Set length */
		length = packet[DCMessageHeader.SIZE + 2] << 8;
		length += packet[DCMessageHeader.SIZE + 3];
		
		/* Parse TLV subobject */
		int msgType = header.getMessageType();
		if(msgType == DCMessageHeader.TYPE_REPLY_ERO){
			/* read ERO TLVS */
			if(type == TLV_TYPE_DC_ERO){
				...
				if(header.getLength() > (4 + length) ){//header + body type + body length field size + body length value
					vtagList = vtagMaskFromBytes(packet, (DCMessageHeader.SIZE + length + 4));
				}else{
			...
			errorCode = 0;
		}else if(msgType == DCMessageHeader.TYPE_REPLY_ERROR){
			if(type == TLV_TYPE_DC_ERROR_CODE){
				errorCode = (packet[DCMessageHeader.SIZE + 4] << 24);
				errorCode += (packet[DCMessageHeader.SIZE + 5] << 16);
				errorCode += (packet[DCMessageHeader.SIZE + 6] << 8);
				errorCode += packet[DCMessageHeader.SIZE + 7];
			}
			ero = new ArrayList<EROSubobject>();//convenience so don't have to worry about null condition
		}else{
	...
	/**
	 * Return the explicit route object contained within a packet returned by the DC API server
	 * @param packet the packet returned by the DC API server in byte form
	...
	private ArrayList<EROSubobject> EROFromBytes(byte[] packet) throws UnknownHostException{
		...
		while((length - bytesRead) > 0){
			/* Get loose bit */
			byte looseVal = (byte) ((packet[DCMessageHeader.SIZE + 4 + bytesRead] & 128) >> 7);
			boolean loose = (looseVal == 1);
			
			/* get type */
			byte type = (byte) (packet[DCMessageHeader.SIZE + 4+ bytesRead] & 127);
			
			/* get length */
			byte tlvLength = packet[DCMessageHeader.SIZE + 5+ bytesRead];
			...
			if(type == EROSubobject.IPV4_PREFIX_SUBOBJ){
				...
				for(int i = 0; i < 4; i++){
					rawIP4[i] = packet[DCMessageHeader.SIZE + 6 + i + bytesRead];
				}
				InetAddress ip4 = InetAddress.getByAddress(rawIP4);
				
				/* Get address length */
				byte addressLength = packet[DCMessageHeader.SIZE + 10 + bytesRead];
				
				/* Get address padding */
				byte padding = packet[DCMessageHeader.SIZE + 11 + bytesRead];
				...
				reserved[0] = packet[DCMessageHeader.SIZE + 6 + bytesRead];
				reserved[1] = packet[DCMessageHeader.SIZE + 7 + bytesRead];
				
				/* get IP address  */
				byte[] rawIP4 = new byte[4];
				for(int i = 0; i < 4; i++){
					rawIP4[i] = packet[DCMessageHeader.SIZE + 8 + i + bytesRead];
				}
				InetAddress ip4 = InetAddress.getByAddress(rawIP4);
				
				/* get Interface id */
				int interfaceId = (packet[DCMessageHeader.SIZE + 12 + bytesRead] << 24);
				interfaceId += (packet[DCMessageHeader.SIZE + 13 + bytesRead] << 16);
				interfaceId += (packet[DCMessageHeader.SIZE + 14 + bytesRead] << 8);
				interfaceId += (packet[DCMessageHeader.SIZE + 15 + bytesRead]);
	...
	private ArrayList<Integer> vtagMaskFromBytes(byte[] packet, int offset) {
		...
		/* Read bitmask */
		if(type == TLV_TYPE_DC_REQUEST_VTAG_MASK){
	 ...
	 */
	public DCMessageHeader getHeader() {
		return header;
	}
	 ...
	 */
	public void setHeader(DCMessageHeader header) {
		this.header = header;
	}
