...
/**
 * Contains the parameters of a TERCE API application request.
 * TERCE application requests are used to retrieve a path between
 * two endpoints and optionally available VLAN tags.
 * App2TERCERequests are issued to the TERCE server by TERCE clients.
 *
 * @author Andrew Lake (alake@internet2.edu)
 */
public class App2TERCERequest {
    /* Encoding values */
     ...
     */
    public App2TERCERequest(String src, String dst, byte encoding, byte swcap,
        byte[] gpidNum, float bwidth) throws UnknownHostException {
        int bodyLength = 0;
        /* Set Type */
        type = new byte[2];
        type[0] = 0x00;
        ...
        /* Create header */
        header = new TERCEMessageHeader(TERCEMessageHeader.TYPE_LSPQ,
                bodyLength + 4);
     ...
     * @throws UnknownHostException returned when an invalid source or destination is given
     */
    public App2TERCERequest(String src, String dst, float bwidth)
        throws UnknownHostException {
        this(src, dst, ENCODING_ETHERNET, SWCAP_L2SC, GPID_ETHERNET, bwidth);
    }

    /**
     * Generates a TERCE API message packet (including header) in raw byte form from this object.
     * @return byte array containing a TERCE API message packet (including header)
     */
    public byte[] toBytes() {
     ...
     * @return the TERCE API header of this message
     */
