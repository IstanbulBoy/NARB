...
public class TERCEReply {
     ...
     */
    public TERCEReply(TERCEMessageHeader header, int type, int length,
        ArrayList<EROSubobject> ero, ArrayList<Integer> vtagList, int errorCode) {
        this.header = header;
        this.type = type;
        this.length = length;
        this.ero = ero;
        ...
        if (msgType == TERCEMessageHeader.TYPE_REPLY_ERO) {
            ...
            if (type == TLV_TYPE_TERCE_ERO) {
                ...
                if (header.getLength() > (4 + length)) { //header + body type + body length field size + body length value
                    vtagList = vtagMaskFromBytes(packet,
                            (TERCEMessageHeader.SIZE + length + 4));
                } else {
     ...
     */
    private ArrayList<EROSubobject> EROFromBytes(byte[] packet)
        ...
        while ((length - bytesRead) > 0) {
            /* Get loose bit */
            byte looseVal = (byte) ((packet[TERCEMessageHeader.SIZE + 4 +
                bytesRead] & 128) >> 7);
            boolean loose = (looseVal == 1);

            /* get type */
            byte type = (byte) (packet[TERCEMessageHeader.SIZE + 4 + bytesRead] &
                127);
            ...
            if (type == EROSubobject.IPV4_PREFIX_SUBOBJ) {
                ...
                for (int i = 0; i < 4; i++) {
                    rawIP4[i] = packet[TERCEMessageHeader.SIZE + 6 + i +
                        bytesRead];
                }
                ...
                /* Get address length */
                byte addressLength = packet[TERCEMessageHeader.SIZE + 10 +
                    bytesRead];
                ...
                localERO.add(new IPv4PrefixSubobject(loose, tlvLength, ip4,
                        addressLength, padding));
                ...
                for (int i = 0; i < 4; i++) {
                    rawIP4[i] = packet[TERCEMessageHeader.SIZE + 8 + i +
                        bytesRead];
                }
                ...
                /* get Interface id */
                int interfaceId = (packet[TERCEMessageHeader.SIZE + 12 +
                    bytesRead] << 24);
                interfaceId += (packet[TERCEMessageHeader.SIZE + 13 +
                bytesRead] << 16);
                interfaceId += (packet[TERCEMessageHeader.SIZE + 14 +
                bytesRead] << 8);
                interfaceId += (packet[TERCEMessageHeader.SIZE + 15 +
                bytesRead]);

                bytesRead += tlvLength;

                localERO.add(new UnumInterfaceSubobject(loose, tlvLength,
                        reserved, ip4, interfaceId));
            }
        }
