...
public class DynamicRCE extends RCE implements RCEInterface {
    ...
    public FindPathResponse findPath(FindPath request)
        ...
        /* Contact TERCE */
        try {
            PropertyReader props = new PropertyReader(System.getenv(
                        "TERCEWS_HOME") + "/terce-ws.properties");
            terceRequest = generateTERCEAPIRequest(request.getFindPath());

            TERCEClient client = new TERCEClient(props.getProperty(
                        "terce.address"),
            ...
            throw this.generateRCEException("Problem contacting TERCE server: " +
                e.getMessage());
        }
     ...
     */
    private App2TERCERequest generateTERCEAPIRequest(
        FindPathContent soapRequest) throws UnknownHostException {
        App2TERCERequest terceRequest;
        ...
        if ((encodingString == null) || encodingString.equals("") ||
                encodingString.toLowerCase().equals("ethernet")) {
        ...
        if ((swcapString == null) || swcapString.equals("") ||
                swcapString.toLowerCase().equals("l2sc")) {
        ...
        if ((gpidString == null) || gpidString.equals("") ||
                gpidString.toLowerCase().equals("ethernet")) {
            gpid = App2TERCERequest.GPID_ETHERNET;
        }

        /* Genrate request */
        terceRequest = new App2TERCERequest(srcHost, dstHost, encoding, swcap,
                gpid, bandwidth);
     ...
     */
    private void setTERCEHeaderOptions(FindPathContent soapRequest,
        TERCEMessageHeader header) {
        /* Set vtag */
        String vtagString = soapRequest.getVtag();
        int vtag = 0;

        if ((vtagString == null) || vtagString.equals("") ||
                vtagString.toLowerCase().equals("none")) {
